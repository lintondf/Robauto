...
public class DouglasPeuckerReducer {
     ...
     */
    public static <T extends GeodeticPosition> ArrayList<T> reduceWithTolerance(ArrayList<T> shape, double tolerance)
    {
    	int n = shape.size();
        ...
        ArrayList<T> newShape = new ArrayList<T>(n); // the new shape to return
        for (int i=0; i<n; i++){
        	if (marked[i])
     ...
     */
    private static <T extends GeodeticPosition> void douglasPeuckerReduction(ArrayList<T> shape, boolean[] marked, double tolerance, int firstIdx, int lastIdx)
    {
        ...
        GeodeticPosition firstPoint = shape.get(firstIdx);
        GeodeticPosition lastPoint = shape.get(lastIdx);

        for (int idx = firstIdx + 1; idx < lastIdx; idx++) {
            GeodeticPosition point = shape.get(idx);
     ...
     */
    public static double orthogonalDistance(GeodeticPosition point, GeodeticPosition lineStart, GeodeticPosition lineEnd)
    {

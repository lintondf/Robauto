 ...
 *
 * <p>
 * Open Location Codes are short, ~10 character codes that can be used instead
 ...
 *
 * <p>
 * Create an object with: OpenLocationCode code = new
 * OpenLocationCode("7JVW52GR+2V"); OpenLocationCode code = new
 * OpenLocationCode("52GR+2V"); OpenLocationCode code = new
 * OpenLocationCode(27.175063, 78.042188); OpenLocationCode code = new
 * OpenLocationCode(27.175063, 78.042188, 11);
 *
 * <p>
 * Once you have a code object, you can apply the other methods to it, such as
 * to shorten: code.shorten(27.176, 78.05)
 *
 * <p>
...
// Value of the most significant longitude digit after it has been converted to an integer.
	...
	public static void main(String[] args) {
        OpenLocationCode olc0 = new OpenLocationCode("PQ2F+4G Manning, South Carolina".split(" ")[0]);
        System.out.println(olc0);
        OpenLocationCode olc1 = olc0.recover(33.69513292786795, -80.2117746637233);
        CodeArea ca = olc1.decode();
        System.out.println(ca.getCenterLatitude()+","+ca.getCenterLongitude() );		
        OpenLocationCode olc2 = olc0.recover(33.69519,-80.21134);
        ca = olc2.decode();
        System.out.println(ca.getCenterLatitude()+","+ca.getCenterLongitude() );		
	 ...
	 * upper right corners and the center of the bounding box for the area the code
	 * represents.
	 */
	...
	public OpenLocationCode(double latitude, double longitude, int codeLength) {
		...
		// Latitude 90 needs to be adjusted to be just less, so the returned code can
		// also be decoded.
		...
		long latVal = (long) (Math.round((latitude + LATITUDE_MAX) * LAT_INTEGER_MULTIPLIER * 1e6) / 1e6);
		long lngVal = (long) (Math.round((longitude + LONGITUDE_MAX) * LNG_INTEGER_MULTIPLIER * 1e6) / 1e6);
		...
		this.code = codeBuilder.subSequence(0, Math.max(SEPARATOR_POSITION + 1, codeLength + 1)).toString();
	}
	...
	/**
	 * Encodes latitude/longitude into 10 digit Open Location Code. This method is
	 * equivalent to creating the OpenLocationCode object and getting the code from
	 * it.
	 *
	 * @param latitude  The latitude in decimal degrees.
	 * @param longitude The longitude in decimal degrees.
	 ...
	 * This method is equivalent to creating the OpenLocationCode object and getting
	 * the code from it.
	 *
	 * @param latitude   The latitude in decimal degrees.
	 * @param longitude  The longitude in decimal degrees.
	...
	/**
	 * Decodes {@link OpenLocationCode} object into {@link CodeArea} object
	 * encapsulating latitude/longitude bounding box.
	 *
	...
	public CodeArea decode() {
		...
		// Initialise the values. We work them out as integers and convert them to
		// doubles at the end.
		long latVal = -LATITUDE_MAX * LAT_INTEGER_MULTIPLIER;
		long lngVal = -LONGITUDE_MAX * LNG_INTEGER_MULTIPLIER;
		// Define the place value for the digits. We'll divide this down as we work
		// through the code.
		...
		return new CodeArea(latitudeLo, longitudeLo, latitudeHi, longitudeHi,
				Math.min(clean.length(), MAX_DIGIT_COUNT));
	}

	/**
	 * Decodes code representing Open Location Code into {@link CodeArea} object
	 * encapsulating latitude/longitude bounding box.
	 *
	 * @param code Open Location Code to be decoded.
	 * @return A CodeArea object.
	 * @throws IllegalArgumentException if the provided code is not a valid Open
	 *                                  Location Code.
	 */
	...
	/**
	 * Returns whether the provided Open Location Code is a short Open Location
	 * Code.
	 *
	 * @param code The code to check.
	 * @return True if it is short.
	...
	/**
	 * Returns whether the provided Open Location Code is a padded Open Location
	 * Code, meaning that it contains less than 8 valid digits.
	 *
	 * @param code The code to check.
	...
	/**
	 * Returns short {@link OpenLocationCode} from the full Open Location Code
	 * created by removing four or six digits, depending on the provided reference
	 * point. It removes as many digits as possible.
	 *
	...
	public OpenLocationCode shorten(double referenceLatitude, double referenceLongitude) {
		...
		double range = Math.max(Math.abs(referenceLatitude - codeArea.getCenterLatitude()),
				Math.abs(referenceLongitude - codeArea.getCenterLongitude()));
		// We are going to check to see if we can remove three pairs, two pairs or just
		// one pair of
	...
	/**
	 * Returns an {@link OpenLocationCode} object representing a full Open Location
	 * Code from this (short) Open Location Code, given the reference location.
	 *
	 * @param referenceLatitude  Degrees.
	 * @param referenceLongitude Degrees.
	...
	public OpenLocationCode recover(double referenceLatitude, double referenceLongitude) {
		...
		String recoveredPrefix = new OpenLocationCode(referenceLatitude, referenceLongitude).getCode().substring(0,
				digitsToRecover);
		...
		// Work out whether the new code area is too far from the reference location. If
		// it is, we
		...
		// Move the recovered latitude by one precision up or down if it is too far from
		// the reference,
		...
		// Move the recovered longitude by one precision up or down if it is too far
		// from the
		// reference.
	...
	/**
	 * Returns whether the bounding box specified by the Open Location Code contains
	 * provided point.
	 *
